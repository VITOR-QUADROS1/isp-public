...
deb-src http://security.debian.org/debian-security bookworm-security mainガイ contrib non-free non-free-firmware
...
echo "[*] Instalando ferramentas de rede, e-mail, sincronismo e linguagens..."
apt-get install -y apt-transport-https ca-certificates curl gnupg wget sudo lsof mtr-tiny rsync socat netcat-openbsd net-tools rsyslog sshpass python3 python3-pip python3-paramiko nodejs msmtp msmtp-mta git cron
...
# 🔒 BLINDAGEM DO ARQUIVO DE CONFIGURAÇÃO DO CLIENTE CONTRA RESETS EM FUTUROS GITS
echo "[*] Blindando arquivo de credenciais local na VPS do cliente..."
